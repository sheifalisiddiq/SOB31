...
def add_piece(game, player, row, column):
    ...
    """
    game[row][column] = player #deleted +1 in the bracket in which it was written column
    return game
...
def switch_player(player):
    if player == 1:#added an another "="  sign 
        return 2
    else:
        return 1
...
if __name__ == '__main__':
    ...
    while winner == 0 and moves_exist(game):
        ...
        while not available:#added a colon 
            ...
            available = check_space_empty(game, row,column)#added "column" in the bracket
            if available == False:
                print("That box is already full.. please select another one!")
        game = add_piece(game, player, row, column)
        display_game(game)
        player = switch_player(player)
        winner = check_winner(game)#removed the comment 
